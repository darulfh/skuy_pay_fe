...
import 'package:capstone_flutter/view/v2/paket_data/paket_data_success_v2.dart';
...
class PinPaketDataV2View extends StatefulWidget {
  const PinPaketDataV2View({super.key});

  @override
  State<PinPaketDataV2View> createState() => _PinPaketDataV2ViewState();
}

class _PinPaketDataV2ViewState extends State<PinPaketDataV2View> {
  ...
  void _submitPin() async {
    ...
    await checkPinPayment(null, otpController.text).then((value) async {
      if (value) {
        ...
        await PayPaketData(token, "", paketData.selectPaketData?.id ?? "",
                paketData.phone, "")
            .payPaketData()
            .then((value) {
          if (value != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => const PaketDataSuccessV2View()),
            );
          } else {
            setState(() {
              isLoading = false;
